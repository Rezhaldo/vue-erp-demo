<template>
  <div v-if="orderStore.orders.length !== 0">
    ...
    <div v-if="orderStore.orders.length === 0">No orders found.</div>
  ...
  <ModalOrderDetail v-model="showModal" :order="selectedOrder" />
</template>

<script setup lang="ts">
import { useOrderHistoryStore, type OrderData } from "@/stores/order-history";
import {
  CalendarIcon,
  CubeIcon,
  CurrencyDollarIcon,
...
import { ref } from "vue";
import ModalOrderDetail from "../component/ModalOrderDetail.vue";
...
const openOrder = (order: OrderData | null) => {
  selectedOrder.value = order;
  showModal.value = true;
};

console.log("orderStoreorderStore", orderStore);
</script>
